...
export class ProfileComponent implements OnInit {
  ...
  protected readonly selectedUser = this.route.snapshot.data['selectedUser'] as Profile;
  protected readonly authUser = this.auth.authUser;

  protected actionLoadingId = this.article.actionLoadingId;
  protected isMyProfile = signal(this.selectedUser?.id == this.authUser()?.id);
  ...
  ngOnInit(): void {
    if (this.selectedUser) {
      this.getArticles(this.selectedUser.id);
    }
  };
